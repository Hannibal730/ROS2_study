...
from turtlesim.msg import Pose #터틀심 패키지의 Pose라는 메시지 타입 가져오기. (선형속도, 각속도 포함)
...
def main(args=None): # main함수가 호출될 때 인자로 받을 수 있는 옵션인데, 지금은 None을 값으로 받았기 때문에 특별한 설정 없이 기본적인 ROS2 초기화가 진행됨.
	...
	turtlesim_subscriber=Turtlesimsubscriber() #위에서 정의한 Turtlesimscriber클래스의 인스턴스를 생성
	rp.spin(turtlesim_subscriber) #그 인스턴스를 무한스핀
	
	turtlesim_subscriber.destroy_node() #근데 혹시 토픽에서 발행물이 더 안 날라온 탓에, Turtlesimscribert클래스의 작동이 멈추고, 인스턴스가 생성되지 않는다면
	rp.shutdown() #셧다운
